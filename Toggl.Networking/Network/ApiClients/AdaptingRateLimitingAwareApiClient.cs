...
namespace Toggl.Networking.Network
{
    internal sealed class AdaptingRateLimitingAwareApiClient : IApiClient
    {
        ...
        private const uint assumedBurstSize = 10u;
        ...
        public AdaptingRateLimitingAwareApiClient(IApiClient internalClient, Func<DateTimeOffset> currentTime)
        {
            ...
            leakyBucket = new LeakyBucket(currentTime, normalBucketSize, assumedBurstSize);
        ...
        public async Task<IResponse> Send(IRequest request)
        {
            ...
            if (response.TryGetBucketSizeFromHeaders(out var size))
            {
                ...
                // size is at the normal size (10). When the bucket size is decreased dynamically and is below a threshold (80),
                ...
                lock (leakyBucket)
                {
                    if (size > leakyBucket.Size && response.StatusCode != HttpStatusCode.TooManyRequests)
                    {
                        burstsEnabled = true;
                    }
