...
@class StartTimerIntentResponse;
...
/*!
 @abstract Handling method - Execute the task represented by the StartTimerIntent that's passed in
 @discussion Called to actually execute the intent. The app must return a response for this intent.

 @param  intent The input intent
 @param  completion The response handling block takes a StartTimerIntentResponse containing the details of the result of having executed the intent

 @see  StartTimerIntentResponse
 */
- (void)handleStartTimer:(StartTimerIntent *)intent completion:(void (^)(StartTimerIntentResponse *response))completion NS_SWIFT_NAME(handle(intent:completion:));
...
/*!
 ...
 @param  intent The input intent
 @param  completion The response block contains a StartTimerIntentResponse containing additional details about the intent that may be relevant for the system to show the user prior to handling.

 @see StartTimerIntentResponse
 */
- (void)confirmStartTimer:(StartTimerIntent *)intent completion:(void (^)(StartTimerIntentResponse *response))completion NS_SWIFT_NAME(confirm(intent:completion:));
 ...
 */
typedef NS_ENUM(NSInteger, StartTimerIntentResponseCode) {
    StartTimerIntentResponseCodeUnspecified = 0,
    StartTimerIntentResponseCodeReady,
    StartTimerIntentResponseCodeContinueInApp,
    StartTimerIntentResponseCodeInProgress,
    StartTimerIntentResponseCodeSuccess,
    StartTimerIntentResponseCodeFailure,
 ...
 */
@property (readonly, NS_NONATOMIC_IOSONLY) StartTimerIntentResponseCode code;
